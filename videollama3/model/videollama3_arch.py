...
class Videollama3MetaForCausalLM(ABC):
    ...
    def _compress_visual_tokens(
        ...
        others: Optional[torch.LongTensor] = None,
    ):
        ...
        if others is not None:
            visual_indices = torch.nonzero(
                input_ids == self.config.visual_alarm_token_id
            ).flatten()
            textual_indices = torch.nonzero(
                input_ids == self.config.textual_alarm_token_id
            ).flatten()
            others[::4] = visual_indices
            others[2::4] = textual_indices
        ...
        return mm_features, input_ids, attention_mask, position_ids, labels, others
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        learnable_tokens: Optional[List[torch.LongTensor]] = None,
    ):
        vision_encoder = self.get_vision_encoder()
        # NOTE: text-only situation
        others = None
        if vision_encoder is None or pixel_values is None or input_ids.shape[1] == 1:
            return input_ids, attention_mask, position_ids, past_key_values, None, labels, others
        ...
        if labels is not None:
            _, N_label = labels.shape
            if N == N_label:
                labels = labels.view(B * N)
            else:
                others = labels[:,N:].view(-1).flatten()
                labels = labels[:,:N].view(B * N)
        ...
        if self.config.use_token_compression:
            ...
            mm_features, input_ids, attention_mask, position_ids, labels, others = self._compress_visual_tokens(
                compression_mask, mm_features, input_ids, attention_mask, position_ids, labels, others
            )
        ...
        if learnable_tokens is not None and learnable_tokens[0] is not None:
            visual_selected = input_ids == self.config.visual_alarm_token_id
            textual_selected = input_ids == self.config.textual_alarm_token_id
            inputs_embeds[visual_selected] = learnable_tokens[0].to(inputs_embeds.dtype)
            inputs_embeds[textual_selected] = learnable_tokens[1].to(inputs_embeds.dtype)
        ...
        return None, attention_mask, position_ids, past_key_values, inputs_embeds, labels, others
