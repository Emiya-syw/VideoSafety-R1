...
class Videollama3Processor(ProcessorMixin):
    ...
    _legacy_safety_label_pattern = re.compile(r"\$(?P<visual>[01])\$(?P<textual>[01])")

    def __init__(self, image_processor=None, tokenizer=None, chat_template=None, learnable_tokens=False, **kwargs):
        ...
        self.visual_alarm_token_id = self.tokenizer.convert_tokens_to_ids(VISUAL_ALARM_TOKEN)
        self.textual_alarm_token_id = self.tokenizer.convert_tokens_to_ids(TEXTUAL_ALARM_TOKEN)
        if self.learnable_tokens:
            for token, token_id in (
                (VISUAL_ALARM_TOKEN, self.visual_alarm_token_id),
                (TEXTUAL_ALARM_TOKEN, self.textual_alarm_token_id),
            ):
                encoded = self.tokenizer.encode(token, add_special_tokens=False)
                if encoded != [token_id]:
                    raise ValueError(
                        f"{token} must be registered as one special token before "
                        "enabling learnable alarm tokens."
                    )

    @staticmethod
    def _insert_token_after_last(text, anchor, token):
    ...
    @staticmethod
    def _insert_token_before_last(text, anchor, token):
        index = text.rfind(anchor)
        if index < 0:
            return text
        return text[:index] + token + text[index:]

    def _insert_alarm_tokens(self, prompt):
        """Insert one visual and one textual alarm placeholder.

        The visual placeholder follows the final expanded image token, while the
        textual placeholder is placed at the end of the user turn. For text-only
        samples both placeholders are inserted before ``<|im_end|>``.
        """
        if DEFAULT_IMAGE_TOKEN in prompt:
            prompt = self._insert_token_after_last(
                prompt, DEFAULT_IMAGE_TOKEN, VISUAL_ALARM_TOKEN
            )
        else:
            prompt = self._insert_token_before_last(
                prompt, "<|im_end|>", VISUAL_ALARM_TOKEN
            )
        return self._insert_token_before_last(prompt, "<|im_end|>", TEXTUAL_ALARM_TOKEN)

    @staticmethod
    def _coerce_binary_label(value, name):
        if value is None:
            return None
        ...
        if isinstance(value, str):
            ...
            aliases = {
                str(SAFE_LABEL): SAFE_LABEL,
                str(HARMFUL_LABEL): HARMFUL_LABEL,
                "safe": SAFE_LABEL,
                "harmful": HARMFUL_LABEL,
                "false": SAFE_LABEL,
                "true": HARMFUL_LABEL,
            }
        ...
        if value in (0, 1):
            return int(value)
        raise ValueError(f"{name} must be a binary safety label, got {value!r}")

    def _extract_explicit_safety_labels(self, message):
        # Preferred schema:
        # {"safety_labels": {"visual": 1, "textual": 0}}
        # Flat keys remain supported for already-prepared datasets.
        labels = message.get("safety_labels", {})
        visual = labels.get("visual", message.get("visual_safety_label"))
        textual = labels.get("textual", message.get("textual_safety_label"))
        if visual is None and textual is None:
            return None
        if visual is None or textual is None:
            raise ValueError("Both visual and textual safety labels must be provided.")
        return (
            self._coerce_binary_label(visual, "visual safety label"),
            self._coerce_binary_label(textual, "textual safety label"),
        )

    def _extract_legacy_safety_labels(self, prompt):
        # Compatibility only. New datasets should use explicit safety_labels.
        match = self._legacy_safety_label_pattern.search(prompt)
        if match is None:
            return prompt, None
        labels = (int(match.group("visual")), int(match.group("textual")))
        return self._legacy_safety_label_pattern.sub("", prompt, count=1), labels

    @staticmethod
    def _infer_legacy_safety_labels(prompt):
        # Last-resort compatibility path for old prompt-only annotations. This
        # heuristic should not be relied on for newly generated AT-SFT data.
        visual_harmful = bool(
            re.search(
                r"\bvideo\b.*\bharmful\b.*\b(content|elements)\b",
                prompt,
                flags=re.IGNORECASE | re.DOTALL,
            )
        )
        textual_harmful = bool(
            re.search(
                r"\bquestion\b.*\b(harmful|violates)\b",
                prompt,
                flags=re.IGNORECASE | re.DOTALL,
            )
        )
        return (
            HARMFUL_LABEL if visual_harmful else SAFE_LABEL,
            HARMFUL_LABEL if textual_harmful else SAFE_LABEL,
        )

    @staticmethod
    def _single_token_position(input_ids, token_id, token_name):
        positions = torch.where(input_ids == token_id)[0]
        if positions.numel() != 1:
            raise ValueError(f"Expected exactly one {token_name}, found {positions.numel()}.")
        return positions
    ...
    def _process_text_with_label(
        ...
        **kwargs,
        ...
        image_idx = 0
        alarm_labels = None
        alarm_positions = None
        alarm_message_idx = next(
            (idx for idx, message in enumerate(text) if message["role"] == "user"),
            None,
        )
        for message_idx, message in enumerate(text):
            sequence_offset = sum(ids.numel() for ids in input_ids_list)
            # Expand each visual placeholder to the number of projected visual
            # tokens. Alarm placeholders are inserted only in the first user turn.
            ...
            prompt = "".join(prompt)
            prompt, legacy_labels = self._extract_legacy_safety_labels(prompt)
            if self.learnable_tokens and message_idx == alarm_message_idx:
                alarm_labels = self._extract_explicit_safety_labels(message)
                if alarm_labels is None:
                    alarm_labels = legacy_labels or self._infer_legacy_safety_labels(prompt)
                prompt = self._insert_alarm_tokens(prompt)
            ...
            if self.learnable_tokens and message_idx == alarm_message_idx:
                alarm_positions = (
                    self._single_token_position(
                        input_ids, self.visual_alarm_token_id, VISUAL_ALARM_TOKEN
                    )
                    + sequence_offset,
                    self._single_token_position(
                        input_ids, self.textual_alarm_token_id, TEXTUAL_ALARM_TOKEN
                    )
                    + sequence_offset,
                )
        ...
        if self.learnable_tokens:
            if alarm_positions is None or alarm_labels is None:
                raise ValueError("Alarm tokens require at least one user message.")
            visual_pos, text_pos = alarm_positions
            visual_type = torch.tensor([alarm_labels[0]], dtype=torch.long)
            text_type = torch.tensor([alarm_labels[1]], dtype=torch.long)
        ...
        if self.learnable_tokens:
            # The collator/model distinguish the four metadata values because the
            # labels tensor is exactly four elements longer than input_ids:
            # [AR labels..., visual_pos, visual_label, text_pos, text_label].
            text_inputs = {
                "input_ids": torch.cat(input_ids_list),
                "labels": torch.cat(
                    [targets, visual_pos, visual_type, text_pos, text_type],
                    dim=-1,
                ),
            }
        else:
    ...
    def _process_text_without_label(
        ...
        **kwargs,
        ...
        for i in range(len(text)):
            ...
            if self.learnable_tokens:
                text[i] = self._insert_alarm_tokens(text[i])
