...
class ModelArguments:
    ...
    learnable_tokens: Optional[bool] = field(default=False)
    multi_task: Optional[bool] = field(default=False)
    lambda_1: Optional[float] = field(default=0.1)
    lambda_2: Optional[float] = field(default=0.1)
...
class LazySupervisedDataset(Dataset):
    ...
    def _convert_normal(self, data_dict):
        ...
        safety_labels = data_dict.get("safety_labels")
        if safety_labels is None:
            visual_label = data_dict.get("visual_safety_label")
            textual_label = data_dict.get("textual_safety_label")
            if visual_label is not None or textual_label is not None:
                safety_labels = {
                    "visual": visual_label,
                    "textual": textual_label,
                }
        if safety_labels is not None:
            first_user_message = next(
                message for message in messages if message["role"] == "user"
            )
            first_user_message["safety_labels"] = safety_labels
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        try:
            ...
            # print(data_dict)
            # 字典 input_ids, labels, pixel_values, grid_sizes, merge_sizes
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        learnable_tokens = labels[0].shape[-1] - input_ids[0].shape[-1] == 4
        if learnable_tokens:
            others = torch.cat([t[..., -4:] for t in labels], dim=0)
            labels = [t[..., :-4] for t in labels]
        ...
        if learnable_tokens:
            labels = torch.cat([labels, others], dim=-1)
        
        print("Collator",input_ids.shape, labels.shape, self.vlprocessor.tokenizer.model_max_length)
        batch = dict(
            input_ids=input_ids,
            labels=labels,
...
class DataCollatorWithFlatteningForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict], separator_id=-100) -> Dict[str, torch.Tensor]:
        ...
        if learnable_tokens:
            others_list = torch.cat(others_list)
            new_labels = torch.cat([new_labels, others_list], dim=-1)
        position_ids = torch.cat(position_ids)
        # print(new_labels.shape, others_list, accumu)
...
def train(attn_implementation=None):
    ...
    config.multi_task =  model_args.multi_task
    config.lambda_1 =  model_args.lambda_1
    config.lambda_2 =  model_args.lambda_2
    ...
    if model_args.vision_encoder is not None:
        ...
        if model_args.learnable_tokens:
            configure_alarm_tokens(tokenizer, models=[model])
