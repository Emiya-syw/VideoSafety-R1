    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            if self.args.llm_lr is not None:
                lm_parameters = [
                    name for name, _ in optimized_parameters if "vision_encoder" not in name and "mm_projector" not in name and "alarm_tokens" not in name and "tokens_cls" not in name
                ]
            ...
            if self.args.token_lr is not None:
                token_parameters = [
                    name for name, _ in optimized_parameters if "alarm_tokens" in name
                ]
                decay_token_parameters = [name for name in token_parameters if name in decay_parameters]
                nodecay_token_parameters = [name for name in token_parameters if name not in decay_parameters]
                optimizer_grouped_parameters.extend([
                    {
                        "params": [p for n, p in optimized_parameters if n in decay_token_parameters],
                        "weight_decay": self.args.weight_decay,
                        "lr": self.args.token_lr,
                    },
                    {
                        "params": [p for n, p in optimized_parameters if n in nodecay_token_parameters],
                        "weight_decay": 0.0,
                        "lr": self.args.token_lr,
                    }
                ])
            if self.args.cls_lr is not None:
                cls_parameters = [
                    name for name, _ in optimized_parameters if "tokens_cls" in name
                ]
                decay_cls_parameters = [name for name in cls_parameters if name in decay_parameters]
                nodecay_cls_parameters = [name for name in cls_parameters if name not in decay_parameters]
                optimizer_grouped_parameters.extend([
                    {
                        "params": [p for n, p in optimized_parameters if n in decay_cls_parameters],
                        "weight_decay": self.args.weight_decay,
                        "lr": self.args.cls_lr,
                    },
                    {
                        "params": [p for n, p in optimized_parameters if n in nodecay_cls_parameters],
                        "weight_decay": 0.0,
                        "lr": self.args.cls_lr,
                    }
                ])
